...
$outputString = "<div class = 'row'><div class = 'col-md-2'>\n"
			...
			. "</div><div class = 'col-md-2'>"
			. "<button type='button' class='btn btn-primary btn-lg' onclick = 'initOrder(3)'>Refresh Orders</button>\n"
			. "</div><div class = 'col-md-2'>"
...
if ($result->num_rows != 0){
	while($row = $result->fetch_assoc()){
		...
		. "<tr><td>Status</td>\n"
		. "<td>" . $row["status"] . " <button onclick = 'changeStatusMinus(" . $row["id"] . ", 3)' type = 'button' class = 'btn btn-default'>-</button>\n"
		. "<button onclick = 'changeStatusPlus(" . $row["id"] . ", 3)' type = 'button' class = 'btn btn-primary'>+</button></td></tr>\n"
