...
if ($result->num_rows != 0){
	while($row = $result->fetch_assoc()){
		...
		. "<tr><td>Status</td>\n"
		. "<td>" . $row["status"] . " <button onclick = 'changeStatusMinus(" . $row["id"] . ", 2)' type = 'button' class = 'btn btn-default'>-</button>\n"
		. "<button onclick = 'changeStatusPlus(" . $row["id"] . ", 2)' type = 'button' class = 'btn btn-primary'>+</button></td></tr>\n"
